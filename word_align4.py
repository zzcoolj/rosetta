# Testing word_align with IBMModel4.
"""
...
if __name__ == '__main__':
    ...
    num_iterations = 20
    start = timer()
    model = IBMModel4(aligned_paras, num_iterations)
    end = timer()
    ...
    with open('align_models/ibm4.model', 'wb') as m_file:
        dill.dump(model, m_file)
    with open('align_models/en.wc', 'wb') as wc_file:
        pickle.dump(wc, wc_file)
    write_common_words_translations(model, wc, 200, 'align_models/word-align4.csv')
