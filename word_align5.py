# Testing word_align with IBMModel5.
"""
...
if __name__ == '__main__':
    ...
    num_iterations = 20
    start = timer()
    model = IBMModel5(aligned_paras, num_iterations)
    end = timer()
    timeelapsed = end - start

    with open('align_models/ibm-model-runtimes.csv', 'a', encoding='utf-8') as output_file:
        output_writer = csv.writer(output_file, delimiter='\t')
        output_writer.writerow(["5", str(num_iterations), timeelapsed, socket.gethostname()])
    output_file.close()

    # Save model and word count
    with open('align_models/ibm5.model', 'wb') as m_file:
        dill.dump(model, m_file)
    with open('align_models/en.wc', 'wb') as wc_file:
        pickle.dump(wc, wc_file)
    write_common_words_translations(model, wc, 200, 'align_models/word-align5.csv')
